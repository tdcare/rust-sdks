    ...
    I: Interceptor,
{
    ...
    /// Returns the negotiated codec preferences for this receiver.
    ///
    /// These codecs are set during SDP negotiation via
    /// `set_codec_preferences_from_remote_description` and reflect
    /// the codecs that were mutually negotiated with the remote peer.
    ///
    /// Unlike [`get_parameters`](RTCRtpReceiver::get_parameters), this
    /// method bypasses the cache and fallback to media engine codecs,
    /// returning only the codecs that were explicitly negotiated.
    ///
    /// Returns an empty slice if no SDP negotiation has completed yet.
    pub fn codec_preferences(&self) -> &[RTCRtpCodecParameters] {
        // peer_connection is mutable borrow, its rtp_transceivers won't be resized,
        // so this access through &self is safe.
        self.peer_connection.rtp_transceivers[self.id.0]
            .receiver()
            .as_ref()
            .map(|r| r.get_codec_preferences())
            .unwrap_or(&[])
    }
