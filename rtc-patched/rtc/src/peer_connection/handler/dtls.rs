    ...
    for DtlsHandler<'a>
{
    ...
    fn handle_read(&mut self, msg: TaggedRTCMessageInternal) -> Result<()> {
        if let RTCMessageInternal::Dtls(DTLSMessage::Raw(dtls_message)) = msg.message {
            trace!("recv dtls RAW {:?}", msg.transport.peer_addr);
            ...
            for message in dtls_endpoint.read(
                ...
                match message {
                    EndpointEvent::HandshakeComplete => {
                        ...
                        {
                            log::info!("DTLS handler: handshake complete -> SRTP profile: {:?}, peer: {}", state.srtp_protection_profile(), msg.transport.peer_addr);
                    ...
                    EndpointEvent::ApplicationData(message) => {
                        trace!("recv dtls application RAW {:?}", msg.transport.peer_addr);
            ...
            // Bypass
            trace!("bypass dtls read {:?}", msg.transport.peer_addr);
    ...
    fn handle_write(&mut self, msg: TaggedRTCMessageInternal) -> Result<()> {
        if let RTCMessageInternal::Dtls(DTLSMessage::Raw(dtls_message)) = msg.message {
            trace!("send dtls RAW {:?}", msg.transport.peer_addr);
            ...
            // Bypass
            trace!("Bypass dtls write {:?}", msg.transport.peer_addr);
