    ...
    for IceHandler<'a>
{
    ...
    fn handle_read(&mut self, mut msg: TaggedRTCMessageInternal) -> Result<()> {
            ...
            self.ctx.ice_transport.agent.get_selected_candidate_pair()
        {
            ...
            trace!("bypass ice read {:?}", msg.transport.peer_addr);
    ...
    fn handle_write(&mut self, mut msg: TaggedRTCMessageInternal) -> Result<()> {
        if let Some((local, remote)) = self.ctx.ice_transport.agent.get_selected_candidate_pair() {
            ...
            trace!("Bypass ice write {:?}", msg.transport.peer_addr);

            // Log RTP packet transmission for diagnostics
            if let RTCMessageInternal::Rtp(_) = &msg.message {
                let bytes = msg.message.len();
                debug!(
                    "ICE sending RTP packet: {} bytes to {} (local: {})",
                    bytes, msg.transport.peer_addr, msg.transport.local_addr
                );
            }
            ...
            // Log RTP drops prominently — these are user-data packets that
            // will never reach the SFU until ICE finishes connecting.
            if matches!(&msg.message, RTCMessageInternal::Rtp(_)) {
                log::warn!(
                    "ICE: dropping RTP packet ({} bytes) — no selected candidate pair yet",
                    msg.message.len()
                );
            } else {
                trace!(
