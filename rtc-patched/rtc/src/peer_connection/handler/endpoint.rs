    ...
    I: Interceptor,
{
    ...
    fn handle_rtp_message(
        ...
        trace!("handle_rtp_message {}", transport_context.peer_addr);
        ...
        if let Some(track_id) = self.find_track_id(ssrc, Some(&rtp_packet.header)) {
            ...
            trace!("drop rtp packet ssrc = {}", ssrc);
        }
        Ok(())
    }
    ...
    fn handle_rtcp_message(
        ...
        trace!("handle_rtcp_message {}", transport_context.peer_addr);
        ...
        if let Some(rtcp_ssrc) = rtcp_ssrc {
            if let Some(track_id) = self.find_track_id(rtcp_ssrc, None) {
                ...
                trace!("drop rtcp packet ssrc = {}", rtcp_ssrc);
            }
        } else {
            trace!("drop rtcp packet due to empty ssrc");
        }
    ...
    fn handle_datachannel_open(
        ...
        trace!("data channel is open for {:?}", transport_context);
        self.ctx
            .event_outs
    ...
    fn handle_datachannel_close(
        ...
        trace!("data channel is close for {:?}", transport_context);
        self.ctx
            .event_outs
            .push_back(RTCEventInternal::RTCPeerConnectionEvent(
    ...
    fn handle_datachannel_message(
        ...
        trace!("data channel recv message for {:?}", transport_context);
        self.ctx.read_outs.push_back(TaggedRTCMessageInternal {
            now,
            transport: transport_context,
    ...
    fn handle_undeclared_ssrc(&mut self, rtp_header: &rtp::Header) -> Option<MediaStreamTrackId> {
        let payload_type = rtp_header.payload_type;

        // Iterate over all RTP transceivers to find a receiver whose
        // codec preferences match the incoming payload type. This handles
        // multi-media-section cases where the remote SDP uses recvonly
        // without explicit SSRC declarations (e.g., LiveKit publisher answers).
        for (id, transceiver) in self.rtp_transceivers.iter_mut().enumerate() {
            // Skip receivers that already have track codings bound
            // (already handled by find_track_id_by_ssrc or a previous
