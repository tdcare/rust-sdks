            ...
            _ = timer.as_mut() => {
                ...
                if now > pli_last_sent + Duration::from_secs(2) {
                    ...
                    for (&receiver_id, &media_ssrc) in &rtp_receiver_id2ssrcs {
                        ...
                        trace!("sending PLI rtcp packet with media_ssrc={}", media_ssrc);
            ...
            res = broadcast_rx.recv() => {
                match res {
                            ...
                            if let Some(mut sender) = peer_connection.rtp_sender(sender_id) {
                                ...
                                if let Err(err) = sender.write_rtp(packet.clone()) {
                                    if err != Error::ErrClosedPipe {
                                        trace!("[Viewer {}] sender {:?} write error: {}", viewer_id, sender_id, err);
                                    }
                                } else {
                                    sent_count += 1;
                                    if sent_count % 100 == 0 {
                                        trace!("[Viewer {}] Sent {} packets", viewer_id, sent_count);
                                    }
                                }
                            }
                        }
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(skipped)) => {
                        trace!("[Viewer {}] Lagged, skipped {} messages", viewer_id, skipped);
                    }
